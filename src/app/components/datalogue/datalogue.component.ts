...
export class DatalogueComponent implements OnInit
{
	...
	ngOnInit()
	{
		...
		this._firebaseService.getEquipe().subscribe(equipe =>
		{
			this.equipe = equipe;
		});

		this._firebaseService.getActions().subscribe(actions =>
		{
		...
		this._firebaseService.getProductions().subscribe(productions =>
		{
	...
	updMateriel(matut: MaterielUtile)
	{
		this._firebaseService.updateMaterielUtile(matut.$key, matut);
	...
	addOj(nomoj:string)
	{
		...
		this._firebaseService.addOj(newOj);
	...
	deleteOj(oj:Oj)
	{
		this._firebaseService.deleteOj(oj.$key);
	}
	...
	deleteMatUt(matut: MaterielUtile)
	{
		...
		this._firebaseService.deleteMaterielUtile(matut.$key);
	}
